...
SHARE_BAR = """
<div class="share-bar">
  <span>Mark rows with &#10003; (reviewed) or &#10005; (remove) &mdash; your marks are saved in this browser. To share them, generate the code below, select it, and copy it (Ctrl/Cmd+C) to send to someone with this same page open.</span>
  <button type="button" id="copy-link-btn">Generate marks code</button>
  <input type="text" id="code-output" readonly aria-label="Your marks code — select and copy" style="display:none">
...
(function(){
  ...
  if(copyBtn && codeOutput){
    copyBtn.addEventListener("click", function(){
      ...
      // Artifacts run this page inside a sandboxed iframe that may not grant
      // clipboard-write permission, so navigator.clipboard.writeText can
      // silently reject \\u2014 always show the code as selectable text too,
      // rather than relying on the clipboard API alone.
      ...
      if(navigator.clipboard && navigator.clipboard.writeText){
        navigator.clipboard.writeText(code).then(function(){
          showStatus("Code copied to your clipboard \\u2014 paste it into someone else's \\"Paste a marks code\\" box.");
        }, function(){
          showStatus("Couldn't copy automatically \\u2014 the code is selected below, press Ctrl/Cmd+C to copy it.");
        });
      } else {
        showStatus("The code is selected below \\u2014 press Ctrl/Cmd+C to copy it.");
      }
    });
  }
